...
function dx = q_dynamics(t,x,T,J)
    % dx = dynamics(x,T,J)
    %
    % INPUTS:
    %   x = 7x1 vector [q,w]
...
function p_dot = mrp_derivative(p,w)
    p_dot = 0.25*((1-p'*p)*eye(3) + 2*skew(p) + 2*p*p')*w;
    %p_dot = 0.25*((1-norm(p)^2)*w - 2*cross(w,p) + 2*dot(w,p)*p);
end

function dx = p_dynamics(x,T,J)
    % dx = dynamics(x,T,J)
    %
    % INPUTS:
    %   x = 6x1 vector [p,w]
